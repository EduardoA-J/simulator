...
namespace AvionesPapelVR
{
    public class FlightController : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            float response = 1f - Mathf.Exp(-8f * dt);
            _smoothPitch = Mathf.Lerp(_smoothPitch, _pitchInput, response);
            _smoothRoll = Mathf.Lerp(_smoothRoll, _rollInput, response);
            _pitch = Mathf.Clamp(_pitch - _smoothPitch * _def.pitchSensitivity * authority * dt, -pitchLimit, pitchLimit);
            if (Mathf.Abs(_smoothPitch) < 0.05f)
                _pitch = Mathf.MoveTowards(_pitch, 3f, Mathf.Lerp(3f, 8f, _def.stability) * dt);
