...
namespace AvionesPapelVR
{
    ...
    public class VrGrabPlane : MonoBehaviour
    {
        ...
        {
            if (!_armed || args.isCanceled || GameManager.Instance == null) return;
        ...
        IEnumerator FinishRelease()
        {
            ...
            if (vel.magnitude < minThrowSpeed)
            {
            ...
            _armed = false;
            gm.BeginFlightFromVrThrow(definition, Vector3.ClampMagnitude(vel * throwBoost, maxThrowSpeed), transform.position);
        }
