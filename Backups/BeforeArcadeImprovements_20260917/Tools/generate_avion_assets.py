...
class Mesh:
    ...
    def cylinder(self, cx, cy, cz, radius, height, segments=10, capped=True):
        ...
        if capped:
            ...
            for i in range(segments):
                j = (i + 1) % segments
                self.add_tri(tc, top[i], top[j])
                self.add_tri(bc, bot[j], bot[i])
    ...
    def cone(self, cx, cy, cz, radius, height, segments=10):
        ...
        for i in range(segments):
            j = (i + 1) % segments
            self.add_tri(tip, base[i], base[j])
            self.add_tri(bc, base[j], base[i])
...
def paper_plane(name: str, style: int) -> Mesh:
    ...
    # Top surfaces
    m.add_tri(n, spine_mid, lw)
    m.add_tri(n, rw, spine_mid)
    m.add_tri(spine_mid, lw, lw_tip)
    m.add_tri(spine_mid, rw_tip, rw)
    m.add_tri(spine_mid, lw_tip, lw_rear)
    m.add_tri(spine_mid, rw_rear, rw_tip)
    m.add_tri(spine_mid, lw_rear, spine_tail)
    m.add_tri(spine_mid, spine_tail, rw_rear)

    # Bottom
    m.add_tri(n, lw, keel)
    m.add_tri(n, keel, rw)
    m.add_tri(keel, lw, lw_rear)
    m.add_tri(keel, rw_rear, rw)
    m.add_tri(keel, lw_rear, spine_tail)
    m.add_tri(keel, spine_tail, rw_rear)
