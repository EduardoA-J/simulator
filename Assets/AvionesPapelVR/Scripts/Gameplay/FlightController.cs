...
namespace AvionesPapelVR
{
    public class FlightController : MonoBehaviour
    {
        ...
        public void Possess(GameObject plane, PlaneDefinition def, Camera cam, Vector3 launchVelocity)
        {
            ...
            _rb.linearVelocity = Vector3.ClampMagnitude(launchVelocity, SpeedLimit);
            ...
            SteeringTracked = false;
            _calibrateAt = Time.unscaledTime + 0.45f;
        ...
        void Update()
        {
            ...
            if (vrMode)
            {
                ...
                _rollInput = Mathf.Abs(right.x) > Mathf.Abs(left.x) ? DeadZone(right.x) : DeadZone(left.x);
                ...
                if (mode && !_modeWas)
                {
                    ...
                    _tiltCalibrated = false;
                    _calibrateAt = Time.unscaledTime;
                }
                ...
                {
                    SteeringTracked = true;
                    if (!_tiltCalibrated && Time.unscaledTime >= _calibrateAt)
                    { _neutralTilt = rotation; _tiltCalibrated = true; }
                    if (_tiltCalibrated)
                    {
                        Quaternion relative = Quaternion.Inverse(_neutralTilt) * rotation;
                        Vector3 aim = relative * Vector3.forward;
        ...
        void FixedUpdate()
        {
            ...
            _roll = Mathf.MoveTowards(_roll, -_smoothRoll * bankAmount,
                _def.rollSensitivity * Mathf.Lerp(0.7f, 1.3f, _def.stability) * dt);
            _yaw += _smoothRoll * _def.turnSpeed * authority * dt;
            ...
            float alignment = Mathf.Lerp(2.8f, 4.2f, _def.stability) * authority;
            velocity = Vector3.RotateTowards(velocity, forward * speed, alignment * dt, 0f);
            _rb.linearVelocity = Vector3.ClampMagnitude(velocity, SpeedLimit);
            ...
            if (level != null)
                acceleration += GameManager.Instance.CourseRotation * Vector3.right * (Mathf.Sin(Time.fixedTime * 0.7f) * level.windStrength * (1f - _def.windResistance) * 0.2f);
            if (GameManager.Instance.IsTurboActive)
            {
                var turbo = GameManager.Instance.PowerUp(PowerUpType.Turbo);
                if (turbo != null && speed < SpeedLimit)
                    acceleration += forward * turbo.boostAcceleration;
            }
            _rb.AddForce(acceleration, ForceMode.Acceleration);
            Vector3 coursePosition = GameManager.Instance.CoursePoint(_rb.position);
            if (coursePosition.y < -1f || _flightTime > (level != null ? level.timeLimit : 90f) || Mathf.Abs(coursePosition.x) > 60f || coursePosition.z < -25f || (level != null && coursePosition.z > level.length + 20f))
        ...
        public void ApplyBoost()
        {
            ...
            float speed = Mathf.Min(SpeedLimit, Mathf.Max(Speed * _def.boostMultiplier, Speed + 2f));
