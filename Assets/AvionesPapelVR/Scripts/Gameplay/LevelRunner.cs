...
namespace AvionesPapelVR
{
    ...
    /// </summary>
    public partial class LevelRunner : MonoBehaviour
    {
        ...
        public void BuildLevel(LevelDefinition level, Transform root)
        {
            ...
            if (gm != null)
            {
                ...
                foreach (var motion in holder.GetComponentsInChildren<MovingObstacleMotion>())
                    motion.axis = gm.CourseRotation * motion.axis;
                foreach (var fan in holder.GetComponentsInChildren<WindFanForce>())
                    fan.direction = gm.CourseRotation * fan.direction;
        ...
        {
            if (level.HasCurves) { BuildCurvedGround(level, parent); return; }
        ...
        {
            int count = level.HasCurves ? Mathf.CeilToInt(level.length / 5f) : 12;
            for (int i = 0; i < count; i++)
            {
                ...
                go.transform.position = GroundPoint(x, 0f, z);
                go.transform.rotation = level.GroundRotation(z);
        ...
        void SpawnEnemies(LevelDefinition level, Transform parent)
        {
            ...
            for (int i = 0; i < level.enemyCount; i++)
            {
                ...
                if (roll == 2)
                {
                    ...
                    fan.radius = 3.2f;
                    fan.direction = level.PathRotation(z) * Vector3.back;
        ...
        void SpawnObstacles(LevelDefinition level, Transform parent)
        {
            ...
            for (int i = 0; i < level.obstacleCount; i++)
            {
                ...
                if (moving)
                {
                    ...
                    m.distance = 2.2f;
                    m.axis = level.GroundRotation(z) * Vector3.right;
        ...
        void SpawnPickup(GameObject prefab, CollectiblePickup.Kind kind, Transform parent, Vector3 pos, float scale)
        {
            var go = prefab != null ? Instantiate(prefab, parent) : GameObject.CreatePrimitive(PrimitiveType.Sphere);
            go.transform.SetParent(parent, true);
            go.transform.position = FlightPoint(pos.x, pos.y, pos.z);
        ...
        void SpawnRings(LevelDefinition level, Transform parent)
        {
            ...
            for (float z = 6f; z < level.length - 5f; z += 7f)
            {
                ...
                if (level.HasCurves)
                {
                    ring.transform.SetPositionAndRotation(level.PathPoint(z), level.PathRotation(z));
                    if (index % 2 == 1) AddCheckpoint(parent, z);
                }
        ...
        {
            if (level.HasCurves) { BuildCurvedGuides(level, parent); return; }
